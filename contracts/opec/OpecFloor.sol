...
contract OpecFloor is ReentrancyGuard, TokenManager {
    ...
    modifier onlyHandler() {
        require(isHandler[msg.sender], "OpecFloor: forbidden");
        _;
    }
    ...
    function setBackedSupply(uint256 _backedSupply) public onlyAdmin {
        require(_backedSupply > backedSupply, "OpecFloor: invalid _backedSupply");
        backedSupply = _backedSupply;
    }

    function setMintMultiplier(uint256 _mintMultiplier) public onlyAdmin {
        require(_mintMultiplier > mintMultiplier, "OpecFloor: invalid _mintMultiplier");
        mintMultiplier = _mintMultiplier;
    }

    // mint refers to increasing the circulating supply
    // the OPEC tokens to be transferred out must be pre-transferred into this contract
    function mint(uint256 _amount, uint256 _maxCost, address _receiver) public onlyHandler nonReentrant returns (uint256) {
        require(_amount > 0, "OpecFloor: invalid _amount");
        ...
        require(cost <= _maxCost, "OpecFloor: _maxCost exceeded");
        ...
        IERC20(opec).transfer(_receiver, _amount);
    ...
    function burn(uint256 _amount, uint256 _minOut, address _receiver) public onlyHandler nonReentrant returns (uint256) {
        ...
        require(amountOut >= _minOut, "OpecFloor: insufficient amountOut");

        backedSupply = backedSupply.sub(_amount);

        IMintable(opec).burn(msg.sender, _amount);
