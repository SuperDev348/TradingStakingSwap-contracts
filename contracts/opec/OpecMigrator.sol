...
import "./interfaces/IOpecIou.sol";
import "./interfaces/IAmmRouter.sol";
import "./interfaces/IOpecMigrator.sol";

contract OpecMigrator is ReentrancyGuard, IOpecMigrator {
    ...
    modifier onlyAdmin() {
        require(msg.sender == admin, "OpecMigrator: forbidden");
        _;
    }

    modifier onlySigner() {
        require(isSigner[msg.sender], "OpecMigrator: forbidden");
        _;
    }
    ...
    function initialize(
        ...
        require(!isInitialized, "OpecMigrator: already initialized");
        require(_whitelistedTokens.length == _iouTokens.length, "OpecMigrator: invalid _iouTokens.length");
        require(_whitelistedTokens.length == _prices.length, "OpecMigrator: invalid _prices.length");
        require(_whitelistedTokens.length == _caps.length, "OpecMigrator: invalid _caps.length");
        require(_lpTokens.length == _lpTokenAs.length, "OpecMigrator: invalid _lpTokenAs.length");
        require(_lpTokens.length == _lpTokenBs.length, "OpecMigrator: invalid _lpTokenBs.length");

        isInitialized = true;

        ammRouter = _ammRouter;
        opecPrice = _opecPrice;
    ...
    function migrate(
        ...
        require(isMigrationActive, "OpecMigrator: migration is no longer active");
        require(whitelistedTokens[_token], "OpecMigrator: token not whitelisted");
        require(_tokenAmount > 0, "OpecMigrator: invalid tokenAmount");

        if (hasMaxMigrationLimit) {
            migratedAmounts[msg.sender][_token] = migratedAmounts[msg.sender][_token].add(_tokenAmount);
            require(migratedAmounts[msg.sender][_token] <= maxMigrationAmounts[msg.sender][_token], "OpecMigrator: maxMigrationAmount exceeded");
        }

        uint256 tokenPrice = getTokenPrice(_token);
        uint256 mintAmount = _tokenAmount.mul(tokenPrice).div(opecPrice);
        require(mintAmount > 0, "OpecMigrator: invalid mintAmount");

        tokenAmounts[_token] = tokenAmounts[_token].add(_tokenAmount);
        require(tokenAmounts[_token] < caps[_token], "OpecMigrator: token cap exceeded");
        ...
        if (lpTokens[_token]) {
            ...
            require(tokenA != address(0), "OpecMigrator: invalid tokenA");
            require(tokenB != address(0), "OpecMigrator: invalid tokenB");
        ...
        IOpecIou(iouToken).mint(msg.sender, mintAmount);
    }
    ...
    function signApprove(address _token, address _spender, uint256 _amount, uint256 _nonce) external nonReentrant onlySigner {
        ...
        require(!signedActions[msg.sender][action], "OpecMigrator: already signed");
    ...
    function getTokenPrice(address _token) public view returns (uint256) {
        ...
        require(price != 0, "OpecMigrator: invalid token price");
        return price;
    }

    function getIouToken(address _token) public view returns (address) {
        address iouToken = iouTokens[_token];
        require(iouToken != address(0), "OpecMigrator: invalid iou token");
        return iouToken;
    }
    ...
    function _validateAction(bytes32 _action) private view {
        require(pendingActions[_action], "OpecMigrator: action not signalled");
    }
    ...
    function _validateAuthorization(bytes32 _action) private view {
        ...
        if (count == 0) {
            revert("OpecMigrator: action not authorized");
        }
        require(count >= minAuthorizations, "OpecMigrator: insufficient authorization");
    }

    function _clearAction(bytes32 _action, uint256 _nonce) private {
        require(pendingActions[_action], "OpecMigrator: invalid _action");
