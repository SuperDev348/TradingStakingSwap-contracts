...
contract EsOpecBatchSender {
    using SafeMath for uint256;

    address public admin;
    address public esOpec;

    constructor(address _esOpec) public {
        admin = msg.sender;
        esOpec = _esOpec;
    }

    modifier onlyAdmin() {
        require(msg.sender == admin, "EsOpecBatchSender: forbidden");
        _;
    }
    ...
    function send(
        ...
        for (uint256 i = 0; i < _accounts.length; i++) {
            IERC20(esOpec).transferFrom(msg.sender, _accounts[i], _amounts[i]);
