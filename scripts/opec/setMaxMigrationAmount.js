...
async function main() {
  ...
  const opecMigrator = await contractAt("OpecMigrator", "0x0472F402EA8E301D7595545884Ad4C420E9865d6")
  const gmt = {
    name: "GMT",
  ...
  if (shouldApprove) {
    for (let i = 0; i < tokens.length; i++) {
      ...
      const migratedAmount = await opecMigrator.migratedAmounts(account, token.contract.address)
      ...
      await sendTxn(opecMigrator.setMaxMigrationAmount(account, token.contract.address, totalAmount), message)
    }
  }
}
